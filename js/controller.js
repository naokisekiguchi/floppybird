  ...
  initSensors:function(){
    ...
    return new Promise(function(resolve,reject){
      spawn(function() {
        ...
        if(self.sensorSettings.light){
          yield groveLightInit(self.port,0x29);
        }
        if(self.sensorSettings.accelerometer){
          yield groveAccelerometerInit(self.port,0x53);
        }
        resolve();
      });
    });
  ...
  getSensor:function(){
    ...
    return new Promise(function(resolve,reject){
      spawn(function(){
        ...
        if(self.sensorSettings.light){lux = yield getLight(self.port,0x29);}
        if(self.sensorSettings.accelerometer){accelerometer = yield getAccelerometer(self.port,0x53);}

        // 確認用に console.log に表示
        //console.log("temp: "+temp);
        //console.log("distance: "+distance);
        if(self.sensorSettings.light){console.log("lux: "+lux);}
        if(self.sensorSettings.accelerometer){console.log("accelerometer: " + accelerometer.x + ","+ accelerometer.y + ","+ accelerometer.z);}
        resolve({
          "light":lux,
